...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(stationImagePool,StationImagePool)
{
	...
	connect(ui.btnDelAll, SIGNAL(clicked()), this, SLOT(deleteAllTxt()));
...
void stationImagePool::deleteAllTxt()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModStationImagePool.get()->rowCount();
	...
	if (ret == QMessageBox::Yes)
	{
		...
		{
			ui.m_tblStationImagePool->model()->removeRows(0, 1);
		}
	}
	else {}
}
