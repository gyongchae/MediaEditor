...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(ContentsPool, AudioFilePool)
{
	...
	connect(ui.btnDelAllAudio, SIGNAL(clicked()), this, SLOT(deleteAllAudio()));
	return false;
}
...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(ContentsPool, VideoFilePool)
{
	...
	connect(ui.btnDelAllVideo, SIGNAL(clicked()), this, SLOT(deleteAllVideo()));
...
void ContentsPool::deleteAllAudio()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModAudioFilePool.get()->rowCount();
	int ret = QMessageBox::question(this,
		"Delete All",
		QString("Do you want to delete all? (total %1)")
		.arg(rowCount));
	...
	if (ret == QMessageBox::Yes)
	{
		...
		{
			ui.m_tblAudioFilePool->model()->removeRows(0, 1);
		}
	}
	else {}
}

void ContentsPool::deleteAllVideo()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModVideoFilePool.get()->rowCount();
	int ret = QMessageBox::question(this,
		"Delete All",
		QString("Do you want to delete all? (total %1)")
		.arg(rowCount));
	...
	if (ret == QMessageBox::Yes)
	{
		for (int r = 0; r < rowCount; ++r)
		{
			ui.m_tblVideoFilePool->model()->removeRows(0, 1);
		}
	}
	else {}
}
