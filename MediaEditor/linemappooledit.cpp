...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(LineMapPoolEdit,LineMapPool)
{
	...
	connect(ui.btnDelAll, SIGNAL(clicked()), this, SLOT(deleteAllMap()));
	return false;
}
...
void LineMapPoolEdit::deleteAllMap()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModLineMapPool.get()->rowCount();
	...
	if (ret == QMessageBox::Yes)
	{
		...
		{
			ui.m_tblLineMapPool->model()->removeRows(0, 1);
		}
	}
	else {}
}
