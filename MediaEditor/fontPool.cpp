...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(fontPool,FontPool)
{
	...
	connect(ui.btnDelAll, SIGNAL(clicked()), this, SLOT(delAllFont()));
...
void fontPool::delAllFont()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModFontPool.get()->rowCount();
	...
	if (ret == QMessageBox::Yes)
	{
		...
		{
			ui.m_tblFontPool->model()->removeRows(0, 1);
		}
	}
	else {}
}
