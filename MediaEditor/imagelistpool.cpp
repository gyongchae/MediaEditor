...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(IMAGE_LIST_POOL_CLASS, ImageIndexList)
{
	...
	connect(ui.btnDelAllList, SIGNAL(clicked()), this, SLOT(deleteAllImgList()));
	return false;
}
...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(IMAGE_LIST_POOL_CLASS, ImageIndex)
{
	...
	connect(ui.btnDelAllItem, SIGNAL(clicked()), this, SLOT(deleteAllImgItem()));
...
void imageListPool::deleteAllImgList()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModImageIndexList.get()->rowCount();
	int ret = QMessageBox::question(this,
		"Delete All",
		QString("Do you want to delete all image list? (total %1)")
		.arg(rowCount));
	...
	if (ret == QMessageBox::Yes)
	{
		...
		{
			ui.m_tblImageIndexList->model()->removeRows(0, 1);
		}
	}
	else {}
}

void imageListPool::deleteAllImgItem()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModImageIndex.get()->rowCount();
	int ret = QMessageBox::question(this,
		"Delete All",
		QString("Do you want to delete all iamge items? (total %1)")
		.arg(rowCount));
	...
	if (ret == QMessageBox::Yes)
	{
		for (int r = 0; r < rowCount; ++r)
		{
			ui.m_tblImageIndex->model()->removeRows(0, 1);
		}
	}
	else {}
}
