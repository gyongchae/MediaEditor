...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(editDisplayItemPool, DisplayItemPool)
{
	...
	connect(ui.btnDelAll, SIGNAL(clicked()), this, SLOT(deleteAllList()));
	return false;
}
...
void editDisplayItemPool::deleteAllList()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModDisplayItemPool.get()->rowCount();
	...
	if (ret == QMessageBox::Yes)
	{
		...
		{
			ui.m_tblDisplayItemPool->model()->removeRows(0, 1);
		}
	}
	else {}
}
