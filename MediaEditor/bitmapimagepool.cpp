...
IMPLEMENT_INIT_FUNCTION_FOR_CLASS(bitmapImagePool, BitmapImagePool)
{
	...
	connect(ui.btnDeleteAll, SIGNAL(clicked()), this, SLOT(deleteAllBitmap()));
...
void bitmapImagePool::deleteAllBitmap()
{
	auto *pDM = CDataManage::GetInstance();
	const int rowCount = pDM->m_pModBitmapImagePool.get()->rowCount();
	...
	if (ret == QMessageBox::Yes)
	{
		...
		{
			ui.m_tblBitmapImagePool->model()->removeRows(0, 1);
		}
	}
	else {}
}
