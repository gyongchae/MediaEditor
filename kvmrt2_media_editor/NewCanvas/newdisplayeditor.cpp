...
NewDisplayEditor::NewDisplayEditor(int nRow, int nWidth, int nHeight, int nDuration, QWidget *parent)
	: QDialog(parent)
	, m_canvas(0)
	...
	, m_pDataMapper(0)
{
	...
	m_canvas = new QGLESLCDCanvas(this);
...
NewDisplayEditor::~NewDisplayEditor()
{
}

void NewDisplayEditor::resizeEvent(QResizeEvent *)
{
	if (m_canvas)
		m_canvas->setMinimumSize(m_ScreenSize);
}

void NewDisplayEditor::initTableView()
{
	auto *pDM = CDataManage::GetInstance();
...
void NewDisplayEditor::acceptedChanges()
{
	qDebug() << Q_FUNC_INFO;
...
void NewDisplayEditor::initWidgetMapper(int nRow)
{
	if (!m_pDataMapper)
	{
		auto *pDM = CDataManage::GetInstance();
