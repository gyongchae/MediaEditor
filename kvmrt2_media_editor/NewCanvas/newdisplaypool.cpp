...
#include "newdisplayeditor.h"

NewDisplayPool::NewDisplayPool(QWidget *parent)
	: QDialog(parent)
{
	ui.setupUi(this);
...
NewDisplayPool::~NewDisplayPool()
{
}

IMPLEMENT_INIT_FUNCTION_FOR_CLASS(NewDisplayPool, DisplayItemPool)
{
...
void NewDisplayPool::addDisplayItemPool(bool bInsert)
{
	...
	if (index.isValid() && bAccepted && (nRow < pTM->VECTOR_CLASS(DisplayItemPool).size()))
	{
		...
		// display editor
		NewDisplayEditor tEditor(nRow, pTmpItm->nWidth, pTmpItm->nHeight, pTmpItm->nDuration * 1000, this);
...
void NewDisplayPool::deleteDisplayItemPool()
{
	int nRow;
	auto *pDM = CDataManage::GetInstance();
...
bool NewDisplayPool::eventFilter(QObject *object, QEvent *event)
{
	auto *pDM = CDataManage::GetInstance();
	QModelIndex index;
...
void NewDisplayPool::closeEvent(QCloseEvent * ev)
{
...
void NewDisplayPool::insertmodifyDisplayItemPool(const QModelIndex &index)
{
	auto *pTM = CTableManage::GetInstance();
	int nRow = index.row();
